...
class VisualProjection(nn.Module):
    def __init__(self, visual_dim, dim, drop_rate=0.0):
        ...
        self.linear = Conv1D(in_dim=visual_dim, out_dim=dim, kernel_size=1, stride=1, bias=True, padding=0)#（1024，768）

    def forward(self, visual_features):
        # the input visual feature with shape (batch_size, seq_len, visual_dim)（2，768，1024）
        visual_features = self.drop(visual_features)
        output = self.linear(visual_features)  # (batch_size, seq_len, dim) （2，768，768）
        return output
...
class GlobalSpanModel(nn.Module):
    def __init__(self,inner_dim = 768):
        ...
        self.dense = nn.Linear(768 ,self.inner_dim * 2)
        self.celoss = nn.CrossEntropyLoss()

    def sinusoidal_position_embedding(self, batch_size, seq_len, output_dim): #bs=49,sl=1,od=768
        ...
        embeddings = embeddings   #49,1,768
        return embeddings

    def calc_loss(self,y_true, y_pred):#48,48
        # 1. 取出真实的标签
        ...
        y_pred = y_pred.view(-1) #2304.2304
        if torch.cuda.is_available():
            y_pred = torch.cat((torch.tensor([0]).float().cuda(), y_pred), dim=0)  # 这里加0是因为e^0 = 1相当于在log中加了1,size([5308417])
        else:
    ...
    def get_hidden(self,p_hidden_sep):
        ...
        qw, kw = p_hidden_sep[..., :self.inner_dim], p_hidden_sep[..., self.inner_dim:] #49,1,768

        pos_emb = self.sinusoidal_position_embedding(p_hidden_sep.size(0), p_hidden_sep.size(1), self.inner_dim).to(p_hidden_sep.device) #49,1,768
        # cos_pos,sin_pos: (batch_size, seq_len, 1, inner_dim)
        cos_pos = pos_emb[..., None, 1::2].repeat_interleave(2, dim=-1)
        sin_pos = pos_emb[..., None, ::2].repeat_interleave(2, dim=-1)#49,1,1,768
        qw2 = torch.stack([-qw[..., 1::2], qw[..., ::2]], -1)
        qw2 = qw2.reshape(qw.shape)#49,1,384,2
        qw = qw * cos_pos + qw2 * sin_pos#49,49,1,768
        kw2 = torch.stack([-kw[..., 1::2], kw[..., ::2]], -1)
        kw2 = kw2.reshape(kw.shape)#49,1,384,2
        kw = kw * cos_pos + kw2 * sin_pos#49,49,1,768

        logit= torch.einsum('bmhd,bnhd->bhmn', qw, kw) #49,1,49,49
        return logit
    def forward(
            self,
            txt_feat, attention_mask, token_types,ious=None,vfeats=None,vfeats_mask=None
    ):


        p_hidden = txt_feat

        # video_features = self.video_affine(vfeats) #输入2，768，1024输出2，768，768
        video_features = vfeats

        features = self.cq_attention(video_features, p_hidden, vfeats_mask, attention_mask) #2，768，768
        features = self.cq_concat(features,p_hidden,attention_mask)#2，768，768

        features = self.highlight_layer(features,vfeats_mask)#2,768,768

        p_hidden_sep1, p_hidden_sep2 = [], []

        token_types_nonzero = token_types.nonzero()
        for i in token_types_nonzero:
            if i[0] == 0:
                txt_hidden = p_hidden[0, i[1]]
                p_hidden_sep1.append(p_hidden[0, i[1]]+features[0,:])


        p_hidden_sep1 = self.get_hidden(p_hidden_sep1) #49,1,49,49

        p_hidden_sep1 = p_hidden_sep1[0, 0, :-1, 1:].contiguous() #48,48
